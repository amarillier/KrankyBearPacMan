...
# fpm-based packager for KrankyBear PacMan
# - macOS: Packages bin/pacman-macos-{arch} as .pkg installer to /Applications
# - Linux: Packages bin/pacman-linux-amd64 as .deb and .rpm installers
# - macOS: Installs as KrankyBearPacMan.app bundle structure
...
Environment variables (optional):
  ...
  URL         Project URL (default: https://github.com/amarillier/KrankyBearPacMan)
  LICENSE     License (default: GNU GPL v3)
  REPLACES    Comma-separated list of packages this replaces (default: pacman)
...
URL=${URL:-"https://github.com/amarillier/KrankyBearPacMan"}
...
build_package() {
  ...
  BIN_NAME="KrankyBearPacMan"
  
  if [[ "$TYPE" == "mac" ]]
  then
    SRC_BIN="bin/pacman-macos-${PKG_ARCH}"
    ...
    SRC_BIN="bin/pacman-linux-amd64"
  ...
  then
    ...
    APP_BUNDLE="KrankyBearPacMan.app"
    ...
    # Create symlink: pacman -> KrankyBearPacMan in Contents/MacOS
    ln -s "$BIN_NAME" "$APP_BUNDLE/Contents/MacOS/pacman"
  ...
  then
    ...
    cp -X "$SRC_BIN" "$STAGING_DIR/pacman"
    cp -XR "$SRC_IMAGES" "$STAGING_DIR/Images"
    cp -XR "$SRC_SOUNDS" "$STAGING_DIR/Sounds"
    
    # Copy ReleaseNotes.txt and rename to ReleaseNotes-pacman.txt
    cp "$SRC_RELEASE_NOTES" "$STAGING_DIR/ReleaseNotes-pacman.txt"
    # Set proper permissions (644 = rw-r--r--)
    chmod 644 "$STAGING_DIR/ReleaseNotes-pacman.txt"
    ...
    SRC_BIN="$STAGING_DIR/pacman"
    # Note: We no longer create a symlink in staging as it was causing fpm issues
    # (symlink and binary both targeting /opt/local/bin/pacman caused "directory" errors)
    SRC_SYMLINK=""
    SRC_IMAGES="$STAGING_DIR/Images"
    SRC_SOUNDS="$STAGING_DIR/Sounds"
    SRC_RELEASE_NOTES_STAGED="$STAGING_DIR/ReleaseNotes-pacman.txt"
  ...
  COMMON_ARGS=(
    ...
    --url "$URL"
    --license "$LICENSE"
    --description "KrankyBear PacMan - A cross-platform PacMan like game"
    -f
  )
  ...
  then
    ...
    PKG_OUTFILE="$OUTDIR/pacman_${VERSION}-${ITERATION}_${PKG_ARCH}.pkg"
    echo "Building macOS .pkg ($PKG_ARCH) -> $PKG_OUTFILE..."
    
    # macOS .app bundle structure:
    # /Applications/KrankyBearPacMan.app/Contents/Info-plist.txt (sample Info.plist)
    # /Applications/KrankyBearPacMan.app/Contents/MacOS/KrankyBearPacMan (executable)
    # /Applications/KrankyBearPacMan.app/Contents/MacOS/Resources/Images (resources)
    # /Applications/KrankyBearPacMan.app/Contents/MacOS/Resources/Sounds (sounds)
    # Note: App looks for Resources/Images and Resources/Sounds relative to executable
    APP_NAME="KrankyBearPacMan.app"
    ...
    FPM_FILES=(
      ...
      "$APP_BUNDLE/Contents/MacOS/pacman=$MACOS_DIR/pacman"
    ...
    DEB_OUTFILE="$OUTDIR/pacman_${VERSION}-${ITERATION}_${DEB_ARCH}.deb"
    RPM_OUTFILE="$OUTDIR/pacman_${VERSION}-${ITERATION}_${RPM_ARCH}.rpm"
    ...
    if [[ -n "$SRC_SYMLINK" ]]
    then
      RPM_FPM_FILES+=("$SRC_SYMLINK=/opt/local/bin/pacman")
    fi
