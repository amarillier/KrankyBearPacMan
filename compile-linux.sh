...
echo "KrankyBear PacMan - Linux Compile Script"
echo "========================================="
echo ""
...
GOOS=linux GOARCH=amd64 CGO_ENABLED=1 go build -ldflags="-s -w" -trimpath -o bin/pacman-linux-amd64
AMD64_BUILD_EXIT=$?
...
then
    ...
    set +e
    CC=aarch64-linux-gnu-gcc GOOS=linux GOARCH=arm64 CGO_ENABLED=1 go build -ldflags="-s -w" -trimpath -o bin/pacman-linux-arm64 2>&1
    ARM64_BUILD_EXIT=$?
    set -e  # Re-enable exit-on-error
...
echo ""
echo "Done. Binaries:"
ls -lh bin/pacman-linux-* 2>/dev/null || echo "  (no binaries found)"
...
if [ -f bin/pacman-linux-amd64 ]
then
    cp bin/pacman-linux-amd64 ./template
fi
...
then
    ...
    then
        echo "AMD64 binary:"
        ldd bin/pacman-linux-amd64 | grep -E "libasound|libGL|libX11" || true
    fi
...
send_binaries() {
    ...
    then
        ...
        local binaries=$(ls bin/pacman-linux-* 2>/dev/null)
    ...
    then
        ...
        echo "  scp $remote_user@$remote_host:$(pwd)/bin/pacman-linux-* ./bin/"
        echo ""
