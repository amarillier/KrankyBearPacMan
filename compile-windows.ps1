# Windows compile script for KrankyBearPacMan / pacman-windows (Windows build + optional packaging)
...
Write-Host "KrankyBear PacMan - Windows Compile Script" -ForegroundColor Cyan
...
Remove-Item -Path (Join-Path bin 'pacman') -Force -ErrorAction SilentlyContinue
...
if ($All -or $Windows -or (-not $Linux -and -not $Current)) {
    ...
    go build -ldflags="$ldflags" -trimpath -o bin/pacman-windows.exe
    if ($LASTEXITCODE -eq 0) {
        ...
        if ($Package) {
            ...
            try {
                Copy-Item -Path (Join-Path $PSScriptRoot "bin/pacman-windows.exe") -Destination (Join-Path $PSScriptRoot "pacman-windows.exe") -Force
            } catch {
            ...
            if (Test-Path $InnoPath) {
                & "$InnoPath" "Inno/KrankyBearPacMan.iss"
                if ($LASTEXITCODE -eq 0) {
...
if ($All -or $Linux) {
    if ($env:OS -eq 'Windows_NT') {
        ...
        $env:GOOS = "linux"
        $env:GOARCH = "amd64"
        go build -ldflags="-s -w" -trimpath -o bin/pacman-linux
        if ($LASTEXITCODE -eq 0) {
...
if ($Current) {
    ...
    go build -ldflags="-s -w" -trimpath -o bin/pacman-windows
    if ($LASTEXITCODE -eq 0) {
...
if ($buildFailed) {
    ...
    Get-ChildItem -Path bin -Filter "*pacman*" | Format-Table Name, Length -AutoSize
    ...
    if ($All -or $Windows -or (-not $Linux -and -not $Current)) {
        ...
        Write-Host "  go-winres extract bin\pacman-windows.exe --dir extracted_resources" -ForegroundColor Cyan
    }
}
