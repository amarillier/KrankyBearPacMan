...
echo "KrankyBear PacMan - Windows Compile (bash)"
echo "================================================"
echo
...
rm -f bin/pacman-windows.exe || true
...
if ! go build -ldflags="$LDFLAGS" -trimpath -o bin/pacman-windows.exe
then
...
then
  ...
  cp -f bin/pacman-windows.exe ./KrankyBearPacMan.exe
  ISCC_DEFAULT="/c/Program Files (x86)/Inno Setup 6/ISCC.exe"
  if [[ -x "$ISCC_DEFAULT" || -f "$ISCC_DEFAULT" ]]
  then
    "${ISCC_DEFAULT}" ./Inno/KrankyBearPacMan.iss || {
